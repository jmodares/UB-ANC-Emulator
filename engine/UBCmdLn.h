 ...
 *
 * The main entry point is UBCmdLn
 */
/**
 * \ingroup commandline
 ...
 * UBCmdLn::Usage, and arguments with UBCmdLn::AddValue.
 * POD argument variables will be set directly; more general arguments
 * can be processed via a Callback.
 *
 * UBCmdLn also provides handlers for these standard arguments:
 * \verbatim
 ...
 * is given below.
 *
 * Finally, UBCmdLn processes Attribute and GlobalValue arguments.
 ...
 * settable throught UBCmdLn, as done by the example below.
 * 
 *
 * UBCmdLn can set the initial value of every attribute in the system
 * with the 
 ...
 * method.
 *
 * UBCmdLn can also set the value of every GlobalValue
 ...
 *  
 *    UBCmdLn cmd;
 *    cmd.Usage ("UBCmdLn example program.\n"
 *               "\n"
 *               "This little program demonstrates how to use UBCmdLn.");
...
public:
   ...
   * Copy constructor
   *
   * \param [in] cmd The UBCmdLn to copy from
   */
  UBCmdLn (const UBCmdLn &cmd);
  /**
   * Assignment
   *
   * \param [in] cmd The UBCmdLn to assign from
   * \return The UBCmdLn
   */
...
private:
   ...
   * Copy constructor
   *
   * \param [in] cmd UBCmdLn to copy
   */
  void Copy (const UBCmdLn &cmd);
 ...
 * \brief Helpers for UBCmdLn to specialize on bool
 */
namespace UBCmdLnHelper {

  /**
   * \ingroup commandlinehelper
   * \brief Helpers to specialize UBCmdLn::UserItem::Parse() on bool
   *
  ...
  /**
   * \ingroup commandlinehelper
   * \brief Helper to specialize UBCmdLn::UserItem::GetDefault() on bool
   *
...
template <typename T>
bool
UBCmdLn::UserItem<T>::HasDefault () const
{
  return true;
}
...
template <typename T>
std::string
UBCmdLn::UserItem<T>::GetDefault () const
{
  return UBCmdLnHelper::GetDefault<T> (*m_valuePtr);
}

template <typename T>
std::string
UBCmdLnHelper::GetDefault (const T & val)
{
...
template <typename T>
bool
UBCmdLn::UserItem<T>::Parse (const std::string value)
{
  return UBCmdLnHelper::UserItemParse<T> (value, *m_valuePtr);
}

template <typename T>
bool
UBCmdLnHelper::UserItemParse (const std::string value, T & val)
{
 ...
 * (shortcut for UBCmdLn::PrintHelper)
 *
 * \see UBCmdLn::PrintHelper
 *
 * Example usage:
 * \code
 *    UBCmdLn cmd;
 ...
 * \param [in] cmd The UBCmdLn describing the program.
 * \returns The stream.
 */
std::ostream & operator << (std::ostream & os, const UBCmdLn & cmd);
